...
\begin{enumerate}
\item First clone the GitHub repository and navigate to the directory.
\begin{lstlisting}[style=withoutNumbering, language=bash]
git clone https://github.com/cssr4africa/software-Installation-scripts.git
\end{lstlisting} 
\item Make the all the shell files in the Software Installation Scripts executable
\begin{lstlisting}[style=withoutNumbering, language=bash]
chmod +x $HOME/workspace/software-Installation-scripts/*.sh
\end{lstlisting}
\item Run the shell script for installing ROS-Noetic.
\begin{lstlisting}[style=withoutNumbering, language=bash]
./install_ros_noetic.sh
\end{lstlisting}
\item Run the shell script for installing the necessary packages and setting up the workspace
\begin{lstlisting}[style=withoutNumbering, language=bash]
...
\end{tcolorbox}
\endgroup
\subsection{Method 2: Manually Installation of Development Environment}
...
\end{figure}
Once you've successfully launched the simulator, your simulation environment should be similar to Figure \ref{fig:gazebo-sim}.
...
mv cssr4africa_models/face_detection/models/* $HOME/workspace/pepper_rob_ws/src/cssr4africa/cssr_system/face_detection/models

# Move the person detection models to the models directory:
mv cssr4africa_models/person_detection/models/* $HOME/workspace/pepper_rob_ws/src/cssr4africa/cssr_system/person_detection/models
...
\noindent Upon completing the installation above, the \textbf{src} folder within the development environment's workspace will contain the CSS4Africa software meta-package and all the necessary packages for the project. As of now, only six software have been uploaded in the \texttt{cssr\_system} and \texttt{unit\_tests} directory: \texttt{animateBehvaiour}, \texttt{faceDetection}, \texttt{soundDetection}, \texttt{gestureExecution}, \texttt{overtAttention}, and \texttt{personDetection}. Below is the directory structure of the workspace for reference.
\begin{figure}[ht]
...
\noindent To run the test on the physical robot, change the first line of the actuatorTestConfiguration.ini and sensorTestConfiguration.ini file to \texttt{platform robot}. On the other hand, to run the test on the simulator platform, change the first line of the actuatorTestConfiguration.ini and sensorTestConfiguration.ini file to \texttt{platform simulator}. 
...
\item [Version 1.10]~\\
Updated the installation manual with face and person detection python environment setup.\\
Yohannes Haile.\\    
26 May 2025.
